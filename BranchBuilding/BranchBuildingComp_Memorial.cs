    ...
    public override void PostInitActive()
    {
        ...
        if (parent.Def.IsHonorSymbol && (parent.Branch.IsBranchOfType(Branch.BranchType.Friendly) || parent.RatkinOrder.Relationship >= EsteemHandler.RelationshipKind.Friendly))
        {
            OrderLetterUtility.ReceiveLetter(
                label: "OARO_MemorialInitActiveLabel".Translate(parent.Def.honorDef.Named(OARO_KeyLibrary_FormatArgName.HONORDEF)),
                text: "OARO_MemorialInitActiveText".Translate(parent.Branch.NameColored.Named(OARO_KeyLibrary_FormatArgName.BranchName),
                                                         parent.Label.Named("BuildingLabel"),
                                                         parent.Def.honorDef.Named(OARO_KeyLibrary_FormatArgName.HONORDEF)),
