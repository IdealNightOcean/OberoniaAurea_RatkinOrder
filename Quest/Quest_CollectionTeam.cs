...
public class QuestNode_CollectionTeam : QuestNode
{
    ...
    protected override void RunInt()
    {
        ...
        questPart_CollectionTeam.Branch = branch.GetValue(slate) ?? slate.Get<Branch>(OARO_KeyLibrary_SlateStoreAs.branch);
        questPart_CollectionTeam.DemandType = demandType.GetValue(slate) ?? slate.Get<BranchDemand.DemandType>(OARO_KeyLibrary_SlateStoreAs.demandType);
        questPart_CollectionTeam.Faction = faction.GetValue(slate) ?? questPart_CollectionTeam.Branch?.RatkinOrder.Faction;

        questPart_CollectionTeam.inSignalEnable = QuestGenUtility.HardcodedSignalWithQuestID(inSignalEnable.GetValue(slate)) ?? slate.Get<string>(OARO_KeyLibrary_SlateStoreAs.inSignal);
